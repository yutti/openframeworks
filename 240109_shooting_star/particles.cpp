...
//------------------------------------------------------------------
void particles::setMode(particleMode newMode) {
	mode = newMode;
}
...
void particles::update() {
	...
	else if (mode == PARTICLE_MODE_NOISE_RAIN) {
		// Simulate rain by having particles fall straight down
		float windX = ofSignedNoise(pos.x * 0.003, pos.y * 0.006, ofGetElapsedTimef() * 3);

		frc.x = windX * 0.1;  // Less horizontal movement compared to rain
		frc.y = 0.14;  // Constant downward force, simulating gravity

		vel *= drag;
		vel += frc * 3;

		// If a particle goes below the screen, have it reappear at the top
		if (pos.y + vel.y > ofGetHeight()) {
			pos.y -= ofGetHeight();
		}
	}
	else if (mode == PARTICLE_MODE_NOISE_STAR) {
		float windY = ofSignedNoise(pos.x * 0.006, pos.y * 0.003, ofGetElapsedTimef() * 0.1);

		frc.x = std::abs(ofSignedNoise(uniqueVal, pos.x * 0.006, ofGetElapsedTimef() * 0.2) * 0.09 + 0.1);
		frc.y = std::abs(windY) * 0.01;
		//frc.x = 0.14;
		//frc.y = windY * 0.1;
...
void particles::draw() {
	...
	if (mode == PARTICLE_MODE_ATTRACT) {
		...
		ofSetColor(255, 255, 255);
		ofDrawEllipse(pos.x, pos.y, scale * 2.0, scale * 13.0);
	}
	else if (mode == PARTICLE_MODE_NOISE_STAR) {
		ofSetColor(color.r, color.g, color.b);
		ofDrawCircle(pos.x, pos.y, scale * 1.4);
	}
}
